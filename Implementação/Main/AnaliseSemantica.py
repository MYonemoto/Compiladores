...
class Semantica:
    ...
    def declaracao_variaveis(self, node):
        ...
        for son in self.lista_variaveis(node.child[1]):
            ...
            if self.scope + "-" + son in self.symbols.keys():
                print("WARNING: variavel '" + son + "' já foi declarada")
    ...
    def indice(self, node):
        if len(node.child) == 1:
            ...
            if tipo != "inteiro":
                print("ERRO: aceita-se apenas valor do tipo inteiro")
    ...
    def cabecalho(self, node):
        ...
        if type_corpo != type_func:
            if node.value != "principal":
                ...
                print("WARNING: a funcao " + node.value + " retorna " + type_corpo + " e deveria retornar " + type_func)
